...
export function AssetFilter() {
  ...
  const handleDeleteSelected = async () => {
    const targets = filteredAssets.filter((asset) => selectedIds.has(asset.id));
    if (targets.length === 0) return;

    if (
      !window.confirm(
        `${targets.length} хөрөнгийг устгах уу? Энэ үйлдлийг буцаах боломжгүй.`,
      )
    ) {
      return;
    }
    ...
    for (const asset of targets) {
      ...
      try {
        ...
        failed.push(asset.assetId || asset.id);
      }
    }
    ...
    if (failed.length > 0) {
      toast.error(`${failed.length} хөрөнгө устсангүй.`, { id: toastId });
      return;
    }
      ...
      <div className="space-y-6">
        <div className="flex justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {someSelected ? (
              <Button
                className="gap-2 rounded-sm border border-red-200 bg-red-600 px-3 text-white hover:bg-red-700"
